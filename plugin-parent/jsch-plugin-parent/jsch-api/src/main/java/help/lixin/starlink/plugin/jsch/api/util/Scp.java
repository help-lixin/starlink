package help.lixin.starlink.plugin.jsch.api.util;
...
/**
 * 之所以拷贝一份:Scp出来的原因在于,ant封装的工具,针对密钥这一块要求必须是本地文件来着的.
 */
public class Scp extends SSHBase {
    ...
    /**
     * Sets the location where files will be transferred to. This can either be a remote directory or a local directory.
    ...
    /**
     * Similar to {@link #setFile setFile} but explicitly states that the file is a local file. This is the only way to
     * specify a local file with a @ character.
     *
    ...
    /**
     * Similar to {@link #setFile setFile} but explicitly states that the file is a remote file.
     *
     * @param aFromUri a string representing the source of the copy.
     * @since Ant 1.6.2
     */
    ...
    /**
     * Similar to {@link #setTodir setTodir} but explicitly states that the directory is a local. This is the only way
     * to specify a local directory with a @ character.
     *
     * @param aToUri a string representing the target of the copy.
     * @since Ant 1.6.2
     */
    ...
    /**
     * Similar to {@link #setTodir setTodir} but explicitly states that the directory is a remote.
     *
     * @param aToUri a string representing the target of the copy.
     * @since Ant 1.6.2
     */
    ...
    private void download(final String fromSshUri, final String toPath) throws JSchException, IOException {
        final String file = parseUri(fromSshUri);

        Session session = null;
        try {
            // TODO lixin
            // 此处实际是拿到spring容器中,单例的session.
            session = openSession();
    ...
    private void upload(final List<ResourceCollection> rcs, final String toSshUri) throws IOException, JSchException {
        ...
        try {
            ...
            // TODO lixin
            // 由于session已经交给spring容器管理了,所以,没有必要关闭session了.
            // if (session != null) {
            // session.disconnect();
            // }
        }
    ...
    private void upload(final String fromPath, final String toSshUri) throws IOException, JSchException {
        final String file = parseUri(toSshUri);
        ...
        try {
            ...
            if (!isSftp) {
                message = new ScpToMessage(getVerbose(), compressed, session, getProject().resolveFile(fromPath), file,
                    preserveLastModified);
            } else {
    ...
    private String parseUri(final String uri) {
        ...
        // TODO lixin 不需要验证密码/密钥文件,因为,我的密钥文件是字符文本,而不是FD.
    ...
    private BuildException exactlyOne(final String[] attrs, final String alt) {
        return new BuildException("Exactly one of [%s]%s is required", String.join("|", attrs),
            alt == null ? "" : " or " + alt);
    }
}
