...
public class ServiceTaskProcessor
    ...
    public String onComplete(AbstractFlowNodeBuilder flowNodeBuilder, ServiceTaskDefinition flowNode)
        ...
        Map<String, String> inputParameters = flowNode.getInputParameters();
        if (null != inputParameters && !inputParameters.isEmpty()) {
            Iterator<Map.Entry<String, String>> iterator = inputParameters.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry<String, String> entry = iterator.next();
                String key = entry.getKey();
                String value = entry.getValue();
                serviceTaskBuilder.camundaInputParameter(key, value);
            }
        }
