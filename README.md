...
### 11. 日志落库
有可能Pipline的执行过程会比较耗时,为了增加体验,要求能实时的查看每一个Pipline操作明细,如果,自己实现这功能,需要大量的在业务代码里做记录,最终的解决方案是对Logger进行扩展,并"透明化切入"(无须在XML里配置独立的Appender),开发完全无感知,只要按照正常打日志,然后,日志会进行落库操作,这个过程是异步(Disruptor)的,允许丢失消息.    
